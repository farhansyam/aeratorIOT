@extends('backend.master')
@section('content')
<div class="row">
                    <div class="col s12">
                        <div class="card">
                            <div class="card-content container">
                                <h5 class="card-title">Sensor Ph</h5>
                                <div class="row">
                                    <div class="col s12 m4">
                                        <img src="{{asset('assets/images/ph.png')}}" alt="" width="1000">

                                    </div>
                                    <div class="col s12 m8">
                                        <p class="mt-3">
                                        OPTIMUS merupakan sebuah website yang berisi tentang bagaimana cara menerapkan pola hidup sehat. 
                                        OPTIMUS memiliki berbagai fitur seperti tips hidup sehat, resep ramuan sehat, kalkulator BMI, dan mini game yang bisa dimainkan untuk hiburan.
                                        </p>
                                        <p>
                                        Kamu bisa dengan bebas mengakses semua informasi kesehatan yang terdapat di OPTIMUS kapan saja dan di mana saja. 
                                        OPTIMUS menyediakan layanan konsultasi secara daring dengan dokter umum yang sukarela membantu ANda.
                                        </p>
                                    </div>
                                    <div class="col s12 m12">
                                        <h2 class="card-title section-title" style="margin-top:20px; font-size:20px;  left: 0; bottom: -15px; right: 0; height: 4px; background: #7fad39; margin: 0 auto;"><span style="margin-top:45px;">Kadar Sensor</span></h2>
                                        <img src="" alt="">
                                    </div>


                                </div>
                            </div>
                        </div>
                    </div>
                </div>
<div class="row">
                    <div class="col s12">
                        <div class="card">
                            <div class="card-content container">
                                <h5 class="card-title">Sensor Turbidity</h5>
                                <div class="row">
                                    <div class="col s12 m4">
                                        <img src="{{asset('assets/images/turb.png')}}" alt="" width="500">

                                    </div>
                                    <div class="col s12 m8">
                                        <p class="mt-3">
                                        OPTIMUS merupakan sebuah website yang berisi tentang bagaimana cara menerapkan pola hidup sehat. 
                                        OPTIMUS memiliki berbagai fitur seperti tips hidup sehat, resep ramuan sehat, kalkulator BMI, dan mini game yang bisa dimainkan untuk hiburan.
                                        </p>
                                        <p>
                                        Kamu bisa dengan bebas mengakses semua informasi kesehatan yang terdapat di OPTIMUS kapan saja dan di mana saja. 
                                        OPTIMUS menyediakan layanan konsultasi secara daring dengan dokter umum yang sukarela membantu ANda.
                                        </p>
                                    </div>
                                    <div class="col s12 m12">
                                        <h2 class="card-title" style="margin-top:20px;">Kadar Sensor</h2>
                                        <img src="" alt="">
                                    </div>


                                </div>
                            </div>
                        </div>
                    </div>
                </div>
<div class="row">
                    <div class="col s12">
                        <div class="card">
                            <div class="card-content container">
                                <h5 class="card-title">Sensor Suhu</h5>
                                <div class="row">
                                    <div class="col s12 m4">
                                        <img src="{{asset('assets/images/suhu.png')}}" alt="" width="500">

                                    </div>
                                    <div class="col s12 m8">
                                        <p class="mt-3">
                                        OPTIMUS merupakan sebuah website yang berisi tentang bagaimana cara menerapkan pola hidup sehat. 
                                        OPTIMUS memiliki berbagai fitur seperti tips hidup sehat, resep ramuan sehat, kalkulator BMI, dan mini game yang bisa dimainkan untuk hiburan.
                                        </p>
                                        <p>
                                        Kamu bisa dengan bebas mengakses semua informasi kesehatan yang terdapat di OPTIMUS kapan saja dan di mana saja. 
                                        OPTIMUS menyediakan layanan konsultasi secara daring dengan dokter umum yang sukarela membantu ANda.
                                        </p>
                                    </div>
                                    <div class="col s12 m12">
                                        <h2 class="card-title" style="margin-top:20px;">Kadar Sensor</h2>
                                        <img src="" alt="">
                                    </div>


                                </div>
                            </div>
                        </div>
                    </div>
                </div>
<div class="row">
                    <div class="col s12">
                        <div class="card">
                            <div class="card-content container">
                                <h5 class="card-title">Sensor ORP</h5>
                                <div class="row">
                                    <div class="col s12 m4">
                                        <img src="{{asset('assets/images/orp.png')}}" alt="" width="500">

                                    </div>
                                    <div class="col s12 m8">
                                        <p class="mt-3">
                                        OPTIMUS merupakan sebuah website yang berisi tentang bagaimana cara menerapkan pola hidup sehat. 
                                        OPTIMUS memiliki berbagai fitur seperti tips hidup sehat, resep ramuan sehat, kalkulator BMI, dan mini game yang bisa dimainkan untuk hiburan.
                                        </p>
                                        <p>
                                        Kamu bisa dengan bebas mengakses semua informasi kesehatan yang terdapat di OPTIMUS kapan saja dan di mana saja. 
                                        OPTIMUS menyediakan layanan konsultasi secara daring dengan dokter umum yang sukarela membantu ANda.
                                        </p>
                                    </div>
                                    <div class="col s12 m12">
                                        <h2 class="card-title" style="margin-top:20px;">Kadar Sensor</h2>
                                        <img src="" alt="">
                                    </div>


                                </div>
                            </div>
                        </div>
                    </div>
                </div>
@endsection